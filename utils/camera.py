...
def open_cam_rtsp(uri, width, height, latency):
    ...
    return cv2.VideoCapture(gst_str,cv2.CAP_GSTREAMER )#cv2.CAP_GSTREAMER
...
def grab_img(cam):
    ...
    while cam.thread_running:
        ...
        if cam.img_handle is None:
            cam._reopen()
            logging.warning('Camera: trying to open camera again')
            #break
        time.sleep(0.1)
...
class Camera():
    ...
    def _open(self):
        ...
        #     self._start()
        if self.rtsp:
            logging.info('Camera: using RTSP stream %s' % self.rtsp)
            self.cap = open_cam_rtsp(self.rtsp, 1920, 1080, self.rtsp_latency)
            self._start()
    ...
    def _reopen(self):
        """Reopen the camera in case of failure."""
        logging.info('Camera: attempting to reopen the camera')
        self.release()
        time.sleep(5)  # Give some time before attempting to reopen
        self._open()

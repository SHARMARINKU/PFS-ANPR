...
class Anpr_Inf:
    def __init__(self):
        ...
        self.states = {'AN': 'Andaman and Nicobar', 'AP': 'Andhra Pradesh', 'AR': 'Arunachal Pradesh', 'AS': 'Assam',
                       ...
                       'CH': 'Chandigarh', 'DD': 'Dadra and Nagar Haveli and Daman and Diu', 'DL': 'Delhi', 'GA': 'Goa',
    ...
    def data_match(self, anprnum, conf, valid, plate, frame, vehicle_orientation, ocr_confidence):
        try:
            ...
            for seqnum in self.anpr_data:
                ...
                if (match > self.match_th and time_diff < 3) or (match > 0.92):
                    ...
                    if valid and self.anpr_data[seqnum]['valid']:
                        if conf > self.anpr_data[seqnum]['conf'] or len(anprnum) > len(
                                self.anpr_data[seqnum]['anprnum']):
                            if not self.anpr_data[seqnum]['issaved']:
                                self.anpr_data[seqnum] = {'curtime': time.time(), 'anprnum': anprnum, 'conf': conf,
                                                          'valid': valid, 'issaved': False, 'ocr_conf': ocr_confidence,
                                                          'veh_ort': vehicle_orientation}
            ...
            if not isupdate:
                ...
                self.anpr_data[self.anprseqnumber] = {'curtime': time.time(), 'anprnum': anprnum, 'conf': conf,
                                                      'valid': valid, 'issaved': False,
            ...
            try:
                ...
                for seqnum in self.anpr_data:
                    if time.time() - self.intimedata[seqnum] > 3 and not self.anpr_data[seqnum]['issaved']:
                        ...
                        self.anpr_imgs[seqnum][1], self.anpr_data[seqnum]['anprnum'], self.bb,
                        self.anpr_data[seqnum]['conf'], self.anpr_data[seqnum]['veh_ort'],
                        self.anpr_data[seqnum]['ocr_conf'], self.anpr_imgs[seqnum][0],
                        self.veh_bbox, "0", self.edge_processing_time)).start()  # ,daemon=True
                        logger.info('Anpr data is {}'.format(self.anpr_data[seqnum]))
                        print('Anpr data is {}'.format(self.anpr_data[seqnum]))
    ...
    def loop_and_detect(self, cam, trt_yolo, conf_th,time,date,datetime, ):  # , vis tracker
        ...
        while True:
            ...
            if img is not None and frame_count % 3 == 0:
                ...
                if save_count == 0:
                    ...
                    cv2.rectangle(frame, (crop_frame[0][0], crop_frame[0][1]), (crop_frame[1][0], crop_frame[1][1]),
                                  (0, 255, 0), thickness=2)
                ...
                if boxes != []:
                    ...
                    for box, conf, cls in zip(boxes, confs, clss):
                        if int(box[0]) > roi[0] and roi[1] < int(box[1]) and int(box[2]) < int(
                                self.frame_width - roi[2]) and int(box[3]) < int(self.frame_height - roi[3]):
                            ...
                            if ocr_number is not None:
                                ...
                                if 0 <= len(ocr_number) <= 11:
                                    self.data_match(ocr_number, conf_f, valid, plate, frame, vehicle_orientation,
                                                    ocr_confidence)
                                    vehicle_no = ocr_number

                                    if vehicle_no != last_vehicle_number:
                                        print("vehicle_no:", {} ,  "last_vehicle_number:".format(vehicle_no,last_vehicle_number))
                                        file_name = str(vehicle_no) + ".xlsx"
                                        try:
                                            if os.path.exists(file_name):
                                                wb = load_workbook(file_name)
                                                ws = wb.active
                                                print(f"Existing file '{file_name}' found.")
                                            else:
                                                wb = Workbook()
                                                ws = wb.active
                                                ws.append(
                                                    ["S.No.", "Date", "Time", "Vehicle Number", "Orientation",
                                                     "Plate Color"])
                                                print(f"📄 New file '{file_name}' created.")
                                        except Exception as e:
                                            print("Error in file searching {}".format(e))
...
def main():
    ...
    anprinference.loop_and_detect(cam, trt_yolo, 0.5,time,date,datetime, )  # , vis=vis tracker

...
INTEGRATION_CFG_PATH = os.path.join(BASE_DIR, "config", "integration_config.json")
DEFAULT_LOG_BASE_DIR = "/home/jbmai/PFS/Logs"
DEFAULT_ANPR_REL = "ANPR/anpr.log"

log_base_dir = DEFAULT_LOG_BASE_DIR
anpr_log_relpath = DEFAULT_ANPR_REL
try:
    with open(INTEGRATION_CFG_PATH, "r", encoding="utf-8") as _cf:
        _icfg = json.load(_cf)
    _lcfg = _icfg.get("logging", {})
    log_base_dir = _lcfg.get("base_dir", DEFAULT_LOG_BASE_DIR)
    anpr_log_relpath = _lcfg.get("anpr_log_relpath", DEFAULT_ANPR_REL)
except Exception:
    pass

anpr_log_path = os.path.join(log_base_dir, anpr_log_relpath)
anpr_log_dir = os.path.dirname(anpr_log_path)
file_handler = None
try:
    os.makedirs(anpr_log_dir, exist_ok=True)
    file_handler = TimedRotatingFileHandler(
        anpr_log_path, when="midnight", interval=1
    )
except Exception as e:
    print(f"[WARN] Could not create ANPR file log at '{anpr_log_path}': {e}. File logging disabled.")
...
class Anpr_Inf:
    ...
    def __init__(self):
        ...
        # ANPR dedupe window: avoid storing the same standing vehicle every second.
        self.anpr_repeat_block_sec = 30
        self.last_plate_per_boom = {}
        self.last_plate_time_per_boom = {}
    ...
    @staticmethod
    def _intersection_area_box_roi(box, roi):
        """Pixel area of intersection between detection box and ROI rectangle."""
    ...
    @staticmethod
    def _point_in_roi_rect(pt, roi):
        cx, cy = pt
        rx1, ry1 = roi[0]
        rx2, ry2 = roi[1]
        return rx1 <= cx <= rx2 and ry1 <= cy <= ry2

    def classify_boom_from_rois(self, box, roi1_rel, roi2_rel):
        """
        Assign Boom-1 vs Boom-2 using overlap with each ROI (not full containment).
        Strict 'inside' fails when the plate box straddles ROI edges; those reads were
        incorrectly forced to Boom-2. Prefer larger intersection; then center-in-ROI;
        then split by horizontal gap between ROIs.
        """
        a1 = self._intersection_area_box_roi(box, roi1_rel)
        a2 = self._intersection_area_box_roi(box, roi2_rel)
        if a1 > a2:
            return "Boom-1"
        if a2 > a1:
            return "Boom-2"
        cx, cy = self._box_center(box)
        if self._point_in_roi_rect((cx, cy), roi1_rel):
            return "Boom-1"
        if self._point_in_roi_rect((cx, cy), roi2_rel):
            return "Boom-2"
        mid_x = 0.5 * (float(roi1_rel[1][0]) + float(roi2_rel[0][0]))
        return "Boom-1" if cx < mid_x else "Boom-2"

    def should_save_anpr(self, plate_number, roi_label):
        """
        Save ANPR only when plate changed, or dedupe window elapsed for same plate+line.
        """
        now = time.time()
        key = f"{roi_label}"
        prev_plate = self.last_plate_per_boom.get(key)
        prev_time = self.last_plate_time_per_boom.get(key, 0)

        if prev_plate == plate_number and (now - prev_time) < self.anpr_repeat_block_sec:
            return False

        self.last_plate_per_boom[key] = plate_number
        self.last_plate_time_per_boom[key] = now
        return True
    ...
    def loop_and_detect(self, cam, trt_yolo, conf_th):
        ...
        roi1, roi2 = [[153, 5], [696, 522]], [[787, 7], [1459, 524]]
        ...
        while True:
            ...
            if img is not None and frame_count % 3 == 0:
                ...
                for box, conf, cls in zip(boxes, confs, clss):
                    roi_label = self.classify_boom_from_rois(box, roi1_rel, roi2_rel)
                    ...
                    if ocr_number:
                        if 6 <= len(ocr_number) <= 11:
                            if self.should_save_anpr(ocr_number, roi_label):
                                logger.info("Number Plate: %s (ROI: %s)", ocr_number, roi_label)
                               
                                on_anpr_boom(ocr_number, roi_label)
                            else:
                                logger.debug(
                                    "Skipped duplicate ANPR %s for %s (within %ss)",
                                    ocr_number, roi_label, self.anpr_repeat_block_sec
                                )
...
def main():
    try:
        from qr_anpr_mapping import init_db
        init_db()
        logger.info("QR–ANPR DB initialized.")
    except Exception as e:
        logger.warning("QR–ANPR DB init failed: %s", e)
